...
from Backbone_Hcapsule.MH_layers import *
...
if setting.model == "mobile_Hcaps":
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_f1_macro > best_f1:
            ...
            torch.save(model, f"entire_model_model.pt")
            print("✅ Model saved with improved F1:", round(best_f1, 4))
        else:
            early_stop_counter += 1
            ...
            if early_stop_counter >= setting.PATIENCE:
                print("⛔️ Early stopping triggered.")
                history_filename = f"model_history.pkl"
                with open(history_filename, 'wb') as f:
                    pickle.dump(history, f)
                print(f"♦️♦️history of model model training saved by name {history_filename} ♦️♦️")
                break
    ...
    print(f"♦️♦️ history of training model model saved♦️♦️")

if setting.model not in ["Hcaps", "mobile", "mobile_Hcaps"]:
